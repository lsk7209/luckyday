# Cloudflare D1 원격 데이터베이스 설정 SQL

이 파일의 SQL을 Cloudflare Dashboard에서 실행하세요.
...
```sql
-- 꿈 심볼 개수 확인
SELECT COUNT(*) as count FROM dream_symbol;

-- 샘플 데이터 확인
SELECT slug, name, category, popularity FROM dream_symbol LIMIT 5;
```
...
이제 데이터베이스가 준비되었습니다. 추가 컨텐츠는 나중에 `dream-seed-full.sql` 파일을 사용하여 추가할 수 있습니다.
